...
REGION_KEYWORDS: Dict[str, List[str]] = {
    ...
    "Азовское море": [
        "азовское море", "азовского моря", "азовском море", "азов море",
    ],
    "Чёрное море": [
        "чёрное море", "черное море", "чёрного моря", "черного моря",
        "чёрном море", "черном море",
    ],
    "Гражданские суда": [
        "гражданское судно", "гражданские суда", "гражданского судна",
        "торговое судно", "торговые суда", "сухогруз", "балкер",
    ],
    "Танкеры": [
        "танкер", "танкеры", "танкера", "нефтяной танкер", "нефтетанкер",
    ],
...
NOISE_LINE_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(r"мы\s+(?:также\s+)?в\s+(?:max|мах)\b", re.I),
    re.compile(r"также\s+в\s+(?:max|мах)\b", re.I),
    re.compile(r"найдите\s+нас\s+в\s+(?:max|мах)", re.I),
    re.compile(r"\bmax\.ru\S*", re.I),
    re.compile(r"^[-–—\s]*(?:max|мах)\s*$", re.I),
    re.compile(r"мониторинг\.?\s*рф", re.I),
    ...
    # --- подписи-источники вида "Мониторинг России - @канал" ---
    re.compile(r"мониторинг\s+росси\w*", re.I),
    re.compile(r"мониторинг.*@\w+", re.I),
    re.compile(r"^[-–—\s]*@\w+[-–—\s.,]*$", re.I),
    # --- подписи вида "мониторинг.ру" / "мониторинг рф" отдельной строкой ---
    re.compile(r"^\s*мониторинг\.?\s*(ру|рф|ru|rf)\.?\s*$", re.I),
    # --- явные упоминания конкретных каналов-подписей ---
    re.compile(r"@rus_monitoring", re.I),
    re.compile(r"@\w*monitor\w*", re.I),
    # --- подписи вида "Радар по всей России - @channel" ---
    re.compile(r"радар\s+по\s+всей\s+росси\w*", re.I),
    re.compile(r"@radarrussi\w*", re.I),
    # --- "Обход белых списков - @bot" и подобные рекламные боты ---
    re.compile(r"обход\s+белых\s+списков", re.I),
    re.compile(r"@\w*internet_boost\w*", re.I),
    re.compile(r"@\w*boost\w*bot\w*", re.I),
    # --- общий шаблон подписи-источника: "Текст - @handle" в конце строки ---
    re.compile(r"[-–—]\s*@\w+\s*$"),
]
